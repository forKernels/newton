...
print(f"Mesh: {path.split(chr(92))[-1]}")
print(f"Vertices: {len(verts)}")
print(f"Triangles: {len(faces)}")
print(f"Bounding box: x=[{verts_m[:,0].min():.3f}, {verts_m[:,0].max():.3f}] "
...
if len(components) > 1:
    ...
    for i in range(min(len(components), 5)):
        for j in range(i + 1, min(len(components), 5)):
            ...
            for v in vi[::max(1, len(vi)//100)]:  # sample for speed
                ...
                if d < min_dist:
                    min_dist = d
            print(f"  Components {i}-{j}: closest distance = {min_dist:.6f}m "
    ...
    print(f"  These could be stitched by merging vertices")
