"""
Blender UR5e + LEAP Hand — Armature Generation Only
=====================================================
By David Clabaugh with Claude Code

Generates armature for UR5e + LEAP already in the scene.
Run blender_ur5e_leap_mesh.py FIRST.
"""
...
def generate_armature():
    print("=" * 65)
    print("UR5e + LEAP HAND — ARMATURE GENERATION")
    print("=" * 65)

    arm_root = bpy.data.objects.get("UR5e_Root")
    if arm_root is None:
        print("\nERROR: UR5e_Root not found. Run blender_ur5e_leap_mesh.py first.")
        return

    for obj in bpy.data.objects:
        if obj.type == "ARMATURE" and "LEAP" in obj.name:
    ...
    utils.create_armature("UR5e_LEAP_Armature", combined, arm_root)

    print(f"""
{"=" * 65}
ARMATURE READY — UR5e + LEAP Hand
{"=" * 65}

  UR5e_LEAP_Armature with {len(combined)} bones ({len(arm_chain)} arm + {len(hand_chain)} hand)
  NOTE: Re-parent finger bones to palm for proper hand rig.
""")
