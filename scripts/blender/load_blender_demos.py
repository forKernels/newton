...
class DynamicMovementPrimitive:
    ...
    def generate(self, new_goals: dict = None, new_starts: dict = None,
                 speed_factor: float = 1.0, noise_scale: float = 0.0):
        """
...
def augment_demonstrations(dmp_params: dict, n_augmented: int = 200, 
                           cloth_group_id: int = 0):
    """
    ...
    for i in range(n_augmented):
        ...
        start_noise = {
            name: params["start"] + np.random.uniform(-0.087, 0.087)
            for name, params in dmp.joints.items()
        }
        
        # Randomize goal (cloth position varies on table)
        # Small perturbation to end configuration
        goal_noise = {
            name: params["goal"] + np.random.uniform(-0.05, 0.05)
            for name, params in dmp.joints.items()
        }
...
def main():
    ...
    parser.add_argument("--data_dir", type=str, required=True,
                        help="Directory with exported Blender animations")
    parser.add_argument("--output_dir", type=str, default="./datasets/",
                        help="Output directory for training datasets")
    parser.add_argument("--augment_per_group", type=int, default=200,
                        help="Number of augmented demos to generate per cloth group")
    parser.add_argument("--validate", action="store_true",
                        help="Replay trajectories in Newton for validation")
    ...
    for group in manifest["groups"]:
        ...
        augmented = augment_demonstrations(
            dmp_params, 
            n_augmented=args.augment_per_group,
            cloth_group_id=gid
        )
    ...
    print(f"  ./isaaclab.sh -p train_cloth_policy.py \\")
